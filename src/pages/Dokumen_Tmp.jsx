...
const Dokumen = () => {
  ...
  const handleAction = (id, action) => {
    ...
    // Logika nyata: navigasi ke halaman penandatanganan, detail, atau pengarsipan.
  };
              ...
              filteredDocuments.map((doc) => (
                <tr key={doc.id} className="hover:bg-gray-50 transition duration-150">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span 
                      className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadge(doc.status)}`}
                    >
                      {doc.status}
                    </span>
                  </td>
